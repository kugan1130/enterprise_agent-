...
async def resolve_context(
    ...
    # Simple heuristic: if the history already contains a sentence answering the query, flag it
    is_in_context = False
    lower_history = history.lower() if history else ""
    lowered_query = user_message.lower()
    # Basic patterns for Kugan profession example
    if "kugan" in lowered_query and ("profession" in lowered_query or "engineer" in lowered_query):
        if "kugan is an ai engineer" in lower_history:
            is_in_context = True
    # Add more generic detection later if needed
    resolved_query = user_message
    if history and llm_client and not is_in_context:
        prompt = (
            ...
            "CRITICAL: You must also determine if the answer to the user's latest query is ALREADY present in the Conversation History. "
            "For example, if the history says 'Kugan is an AI Engineer' and the user asks 'What is Kugan's profession?', the answer is in the history.\n\n"
            f"Conversation History:\n{history}\n\n"
            f"User's Latest Query: {user_message}\n\n"
            "Return ONLY a JSON object: {\"resolved_query\": \"<standalone_query>\", \"is_in_context\": true/false}"
        )
        try:
            ...
            is_in_context = data.get("is_in_context", False)
        except Exception:
            resolved_query = user_message
            # is_in_context remains as detected above
